  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat: any, i: number) => {
          ...
          return (
            <div 
              key={stat.name} 
              className="p-6 bg-card border rounded-[2.5rem] space-y-4 shadow-sm"
            >
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-card border rounded-[2.5rem] p-8 space-y-6 shadow-sm">
           <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold">Revenue Trends</h2>
              <div className="flex items-center gap-2 px-3 py-1 bg-muted rounded-lg text-[10px] font-bold text-muted-foreground">
        ...
        <div className="bg-card border rounded-[2.5rem] p-8 space-y-6 shadow-sm flex flex-col">
              ...
              {data?.popularItems?.map((item: any) => (
                <div key={item.id} className="flex items-center gap-4">
                   ...
                   <div className="text-right">
                      <p className="font-bold text-xs">Top Seller</p>
                      <p className="text-[10px] text-green-600 font-bold">+{((item._count.orderItems / 10) * 100).toFixed(1)}%</p>
                   </div>
                </div>
              ))}
           </div>
           <Link 
             ...
             className="w-full py-4 text-center border rounded-2xl text-sm font-bold hover:bg-muted transition-colors mt-4"
           >
              View Analytics
           </Link>
        </div>
